...
def getFrames(filename, first_frame, second_frame):
    ...
    for frame_number in [first_frame, second_frame]:
...
def blockSearchBody(anchor, target, blockSize, searchArea=7):
    ...
    for y in range(0, hSegments * blockSize, blockSize):
        for x in range(0, wSegments * blockSize, blockSize):
            bcount += 1
    ...
    assert bcount == hSegments * wSegments
    return predicted
