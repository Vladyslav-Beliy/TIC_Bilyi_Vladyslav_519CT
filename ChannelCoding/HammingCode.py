...
CHUNK_LENGTH = 8
if CHUNK_LENGTH % 8:
    raise AssertionError("Довжина блоку має бути кратна 8")
CHECK_BITS = [i for i in range(1, CHUNK_LENGTH + 1) if not i & (i - 1)]
with open("results_hamming.txt", "w", encoding="utf-8"):
    pass
...
def encode(source):
    text_bin = get_chars_to_bin(source)
    result = ''
